...
def change_to_newmain(context):
    ...
    if context.portfolio.positions[context.last_main_symbol].sell_quantity !=0:
        buy_close(context.last_main_symbol,context.target_nums)
        print ('close short:', context.now)
        sell_open(context.s1,context.target_nums)
    # 多头平仓
    if context.portfolio.positions[context.last_main_symbol].buy_quantity!= 0:
        sell_close(context.last_main_symbol,context.target_nums)
        print ('close long', context.now)
        buy_open(context.s1,context.target_nums)
    context.main_changed = False
    context.last_main_symbol = context.s1
...
# 2. 根据开盘前计算得到指标，判断是否发出信号
    ...
    C0 = bar_dict[context.s1].close
    close = history_bars(context.s1,3,'1d','close')
    C = history_bars(context.s1,5, '1d','close')
    a1 = close[0]
    a2 = close[1]
    a3 = close[2]
    ...
    #短期指数平均线
    context.AvgValue1 = talib.EMA(C,5)
    
#-------------------多头部分---------------------------------
    #多头开仓
    if sell_qty == 0 and buy_qty == 0:
        if a1 < a2 and a2 < a3 and C0 > context.AvgValue1[-1]:
            buy_open(context.s1,context.target_nums)
    #多头空仓
    if buy_qty != 0:
        if Op0 > C0 and C0 < context.AvgValue1[-1]:
            sell_close(context.s1,context.target_nums)
#-------------------空头部分---------------------------------
    #空头开仓
    ...
    if sell_qty != 0:
        if Op0 < C0 and C0 > context.AvgValue1[-1]:
            buy_close(context.s1,context.target_nums)