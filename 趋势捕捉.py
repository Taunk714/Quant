        ...
        if len(context.future_account.positions.keys())>0:  #已经有仓位
            try:
                if position['side'] == 'SELL' and  position['quantity']>0:  #如果超过买入线，切当时是空头仓位，那么平空仓，买开
                    if (A2_1 > A3_1 and A3_1 > A4_1 and A2_1 > A2_2):
                        buy_close(i,position['quantity'],price = O1[-1] *(1 + context.k))
                        buy_open(i,position['quantity'],price = O1[-1] *(1 + context.k))
                    if A2_1 < A3_1 or A3_1 < A4_1 or A2_1 > A2_1:
                        buy_close(i,position['quantity'],price = O1[-1] *(1 + context.k))
                if position['side'] == 'BUY' and  position['quantity']>0:  #如果低于卖出线，且当时的仓位是多头仓位，那么平多仓，卖开
                    if  (A2_1 < A3_1 and A3_1 < A4_1 and A2_1 < A2_2) :
                        sell_close(i,position['quantity'],price = O1[-1] * (1 - context.k))
                        sell_open(i,position['quantity'],price = O1[-1] * (1 - context.k))
                    if A2_1 > A3_1 or A3_1 > A4_1 or A2_1 < A2_2:
                        sell_close(i,position['qiantity'],price = O1[-1] * (1 - context.k))
            except Exception as e:
                logger.error('[信号出现]下单异常:'+str(e))
        else:                                           #还没有仓位
            if A2_1 > A3_1 and A3_1 > A4_1 and A2_1 > A2_2:
                buy_open(i,context.target_nums,price = O1[-1] * (1 + context.k))   #单仓开仓
            if A2_1 < A3_1 and A3_1 < A4_1 and A2_1 < A2_2:
                sell_open(i,context.target_nums,price = O1[-1] * (1 - context.k))
