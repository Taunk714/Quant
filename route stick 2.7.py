...
def change_to_newmain(context):
    ...
    if context.portfolio.positions[context.last_main_symbol].sell_quantity !=0:
        buy_close(context.last_main_symbol,100)
        print ('close short:', context.now)
        sell_open(context.s1,100)
    # 多头平仓
    if context.portfolio.positions[context.last_main_symbol].buy_quantity!= 0:
        sell_close(context.last_main_symbol,100)
...
#-------------------多头持仓---------------------------------
    ...
    if op[0] > cl[0] and op[1] > cl[1] and  op[2] > cl[2] and buy_qty != 0:
        if cl0 > context.AvgMa5[-1]:
            sell_close(context.s1,100,price = bar_dict[context.s1].close)
            sell_open(context.s1,100,price = bar_dict[context.s1].close)
...
#-------------------空头持仓---------------------------------
    ...
    if op[0] < cl[0] and op[1] < cl[1] and  op[2] < cl[2] and sell_qty != 0:
        if cl0 < context.AvgMa5[-1]:
            buy_close(context.s1,100,price = bar_dict[context.s1].close)
            buy_open(context.s1,100,price = bar_dict[context.s1].close)
        elif cl0 > context.AvgMa5[-1] and cl0 < context.AvgMa10[-1]:
            pass
        else:
            buy_close(context.s1,100,price = bar_dict[context.s1].close)
#-------------------开仓-------------------------------------
    #多头开仓
    if a1 < a2 and a2 < a3 and cl0 > context.AvgMa5[-1] and sell_qty == 0 and buy_qty == 0:
        buy_open(context.s1,100,price = bar_dict[context.s1].close)    
    if a1 > a2 and a2 > a3 and cl0 < context.AvgMa5[-1] and sell_qty == 0 and buy_qty == 0:
        sell_open(context.s1,100,price = bar_dict[context.s1].close)    
